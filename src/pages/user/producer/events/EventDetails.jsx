...
import { authHeader, unSlugify, baseUrl } from '../../../../utils/utils'
...
const EventDetails = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      try {
        ...
        setLoading(true)
        const response = await axios.get(`${baseUrl}/events/${eventId}?expand=createdBy,performedBy,asset,location`, { headers })
  ...
  const getAttachmentExtension = (url = '') => {
    ...
    return fileExt ? `.${fileExt.toLowerCase()}` : '.file'
  }
      ...
      <div className="w-full space-y-4">
        ...
        <div className="rounded-xl p-4 bg-white dark:bg-at-dark-gray/5 shadow-xl shadow-black/5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="p-3 rounded-lg bg-at-dark-gray/5 dark:bg-at-dark-surface">
              <p className="text-xs opacity-60 mb-1">Performed By</p>
              <p className="text-sm font-semibold">{event.performedBy?.name || 'N/A'}</p>
            </div>
            <div className="p-3 rounded-lg bg-at-dark-gray/5 dark:bg-at-dark-surface">
            ...
            <div className="p-3 rounded-lg bg-at-dark-gray/5 dark:bg-at-dark-surface">
              ...
              <p className="text-sm font-semibold">{moment(event.date).format('LL')}</p>
            </div>
            <div className="p-3 rounded-lg bg-at-dark-gray/5 dark:bg-at-dark-surface">
              <p className="text-xs opacity-60 mb-1">Next Due Date</p>
              <p className="text-sm font-semibold">{event.nextDueDate ? moment(event.nextDueDate).format('LL') : 'N/A'}</p>
            </div>
          ...
          {event.attachments?.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {event.attachments?.map((attachment, attachmentIndex) => (
                <div className="p-3 rounded-lg bg-at-dark-gray/5 dark:bg-at-dark-surface flex items-center justify-between gap-x-3" key={attachmentIndex}>
                  <div className="min-w-0 flex-1">
                    {isImageAttachment(attachment.url) ? (
                      ...
                      <div className="w-full h-28 rounded-lg border border-at-dark-gray/20 dark:border-at-dark-gray/40 bg-white/80 dark:bg-at-dark-gray/10 flex items-center justify-center">
                        <p className="text-sm font-semibold uppercase tracking-wide">{getAttachmentExtension(attachment.url)}</p>
                      </div>
                    )}
                  </div>
                  <a
        ...
        <div className="flex gap-x-3">
          ...
          {event?.asset && (
            <Link
              to={`/producer/assets/asset/${event?.asset?.id}`}
